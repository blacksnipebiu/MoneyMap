...
- [ ] 5. 重建 CategoriesViewModel（完整 CRUD）

  **What to do**:
  - 重写 `src/Bookkeeping.App/ViewModels/CategoriesViewModel.cs`
  - 注入 `ICategoryService`（通过 App.Services）
  - 属性：
    - `ObservableCollection<CategoryItem> IncomeCategories` — 收入分类列表
    - `ObservableCollection<CategoryItem> ExpenseCategories` — 支出分类列表
    - `CategoryItem? SelectedCategory` — 当前选中分类
    - `string NewCategoryName` — 新分类名
    - `TransactionType SelectedType` — 新分类类型（默认 Expense）
    - `string? SelectedIcon` — 选中 Emoji 图标
    - `bool IsEditing` — 编辑模式标志
    - `bool IsLoading` — 加载状态
    - `string? ErrorMessage` — 错误消息
  - CategoryItem 是一个简单封装类（Id, Name, Icon, Type, SortOrder, TransactionCount）
  - 命令：
    - `LoadCategoriesCommand` — 加载分类列表，按 Type 分组填充 IncomeCategories 和 ExpenseCategories
    - `AddCategoryCommand` — 创建新分类，验证名称非空且不重复，添加到对应列表
    - `EditCategoryCommand(CategoryItem)` — 进入编辑模式，填充编辑表单
    - `UpdateCategoryCommand` — 保存编辑，验证名称不重复（排除自身）
    - `DeleteCategoryCommand(CategoryItem)` — 检查有关联交易，有则弹出重新分配对话框，无则直接确认删除
    - `MoveUpCommand(CategoryItem)` — SortOrder 上移
    - `MoveDownCommand(CategoryItem)` — SortOrder 下移
    - `SelectIconCommand` — 打开 Emoji 选择器
  - 实现分页或虚拟化（分类数量通常不多，可能不需要）
  - 调用 CategoryService 的 CRUD 方法，结果映射到 CategoryItem

  **Must NOT do**:
  - 不实现 ParentId/Children 相关逻辑（本期不做多级分类）
  - 不修改 Transaction 模型
  - 不在 ViewModel 中直接访问 DbContext

  **Recommended Agent Profile**:
  - **Category**: `deep`
    - Reason: ViewModel 是功能核心，包含业务逻辑和状态管理
  - **Skills**: []

  **Parallelization**:
  - **Can Run In Parallel**: NO
  - **Parallel Group**: Wave 2 (starts after Task 1)
  - **Blocks**: Tasks 6, 7, 8, 9, 10, 11
  - **Blocked By**: Task 1 (CategoryService), Task 3 (EmojiData)

  **References**:

  **Pattern References**:
  - `src/Bookkeeping.App/ViewModels/ImportViewModel.cs` — 现有 ViewModel 模式，使用 ObservableCollection、CommunityToolkit.Mvvm 的 [ObservableProperty] 和 [RelayCommand]
  - `src/Bookkeeping.App/ViewModels/MainWindowViewModel.cs` — ViewModel 缓存和导航模式，了解如何获取 IServiceProvider

  **API/Type References**:
  - `src/Bookkeeping.Core/Services/ICategoryService.cs` — 要调用的 Service 方法签名
  - `src/Bookkeeping.Core/Models/Category.cs` — Category 模型字段

  **WHY Each Reference Matters**:
  - ImportViewModel.cs：最复杂的 ViewModel，展示了项目中的标准模式（异步加载、[ObservableProperty]、[RelayCommand]、Toast 通知）
  - ICategoryService.cs：ViewModel 调用的所有方法签名必须匹配
  ...
  **QA Scenarios (MANDATORY)**:
  ...
  Scenario: 加载分类列表
    Tool: interactive_bash (dotnet run)
    Preconditions: 数据库有种子分类
    Steps:
      1. 启动应用，导航到分类管理页面
      2. 验证收入和支出分类分别显示在两个分组中
      3. 验证每个分类显示名称、图标、交易数量
    Expected Result: 分类列表正确分组显示
    Failure Indicators: 列表为空、分组错误、缺少字段
    Evidence: .sisyphus/evidence/task-5-load-categories.png

  Scenario: 添加分类 - 重复名称
    Tool: interactive_bash (dotnet run)
    Preconditions: 数据库已有 "餐饮美食" 分类
    Steps:
      1. 点击添加分类
      2. 输入名称 "餐饮美食"，选择类型为支出
      3. 点击确认
      4. 验证显示错误提示 "该类型下已存在同名分类"
    Expected Result: 添加被拒绝，显示明确错误信息
    Failure Indicators: 分类被成功创建（不应发生）
    Evidence: .sisyphus/evidence/task-5-duplicate-name.png
  ```

  **Commit**: YES (group with Wave 2)
  - Message: `feat(category): rebuild CategoriesViewModel with full CRUD`
  - Files: `src/Bookkeeping.App/ViewModels/CategoriesViewModel.cs`

- [ ] 6. 重建 CategoriesView.axaml（数据绑定 UI）

  **What to do**:
  - 重写 `src/Bookkeeping.App/Views/CategoriesView.axaml` 和 `.axaml.cs`
  - 布局设计：
    - 顶部标题栏："分类管理"
    - 两个 Tab 或 GroupBox：收入分类 / 支出分类
    - 每个分类列表项显示：Emoji 图标 + 名称 + 交易数量 + 编辑/删除按钮
    - 底部添加区域：类型选择（收入/支出）+ 名称输入 + Emoji 选择 + 添加按钮
    - 编辑模式：行内编辑或弹窗（建议弹窗，与添加共用）
  - 数据绑定：
    - IncomeCategories → 收入分组 ListBox/ItemsControl
    - ExpenseCategories → 支出分组 ListBox/ItemsControl
    - SelectedCategory → 双向绑定
    - 所有按钮绑定对应 Command
  - 使用现有 Converter（TransactionTypeBrushConverter 等）保持风格一致
  - 空状态：当分类列表为空时显示友好的空状态提示
  - 响应式布局：适配窗口大小变化

  **Must NOT do**:
  - 不使用硬编码的分类列表（必须绑定 ViewModel 数据）
  - 不实现拖拽排序（使用上移/下移按钮）
  - 不引入新的 NuGet 包（只用项目已有的 Avalonia 包）

  **Recommended Agent Profile**:
  - **Category**: `visual-engineering`
    - Reason: 需要设计 UI 布局和样式
  - **Skills**: [`/avalonia-dev`]

  **Parallelization**:
  - **Can Run In Parallel**: YES
  - **Parallel Group**: Wave 2 (depends on Task 5 ViewModel)
  - **Blocks**: Task 11
  - **Blocked By**: Task 5
  ...
  **Pattern References**:
  - `src/Bookkeeping.App/Views/ImportView.axaml` — 现有最复杂的 View，展示 Avalonia UI 模式
  - `src/Bookkeeping.App/App.axaml` — 应用主题和 Converter 注册

  **API/Type References**:
  - `src/Bookkeeping.App/ViewModels/CategoriesViewModel.cs` — 完成的 ViewModel，绑定到其属性和命令

  **WHY Each Reference Matters**:
  - ImportView.axaml：参考分类映射 UI 的样式和布局模式
  - App.axaml：复用已注册的 Converter
  ...
  **QA Scenarios (MANDATORY)**:
  ...
  Scenario: 分类列表正确渲染
    ...
    Steps:
      1. 启动应用，导航到分类管理
      2. 验证收入和支出分组正确显示
      3. 验证每个分类项包含图标、名称、操作按钮
      4. 验证列表从数据库加载而非硬编码
    Expected Result: 分类列表正确渲染，数据来自 ViewModel
    Failure Indicators: 列表为空、数据硬编码、缺少操作按钮
    Evidence: .sisyphus/evidence/task-6-categories-ui.png

  Scenario: 删除分类 - 有交易需重新分配
    Tool: interactive_bash (dotnet run)
    Preconditions: 分类 "餐饮美食" 下有 3 条交易
    Steps:
      1. 点击 "餐饮美食" 的删除按钮
      2. 验证弹出重新分配对话框
      3. 在对话框中选择目标分类 "购物消费"
      4. 点击确认
      5. 验证 "餐饮美食" 从列表消失
      6. 验证之前的 3 条交易现在分类为 "购物消费"
    Expected Result: 删除成功，交易重新分配到目标分类
    Failure Indicators: 删除失败、交易分类丢失、重新分配对话框未弹出
    Evidence: .sisyphus/evidence/task-6-delete-reassign.png
  ```

  **Commit**: YES (group with Wave 2)
  - Message: `feat(category): rebuild CategoriesView with data-bound UI`
  - Files: `src/Bookkeeping.App/Views/CategoriesView.axaml`, `src/Bookkeeping.App/Views/CategoriesView.axaml.cs`

- [ ] 7. 分类编辑弹窗

  **What to do**:
  - 创建 `src/Bookkeeping.App/Views/CategoryEditDialog.axaml` 和 `.axaml.cs`
  - 创建 `src/Bookkeeping.App/ViewModels/CategoryEditViewModel.cs`
  - 弹窗内容：
    - 分类名称 TextBox（最大 20 字符，验证非空）
    - 分类类型选择（收入/支出，编辑时不可更改）
    - Emoji 图标选择器（集成 EmojiPickerButton）
    - SortOrder 显示（只读）
    - 确认/取消按钮
  - 弹窗使用 Avalonia Window 弹出
  - 编辑模式：类型不可更改，名称可修改
  - 添加模式：类型可选择（默认当前 Tab 的类型）
  - 验证：名称非空、同类型下不重复

  **Must NOT do**:
  - 不允许编辑时更改分类类型（可能导致交易关联混乱）
  - 不使用内联编辑（使用弹窗更安全）
  - 不添加动画效果（保持简单）
  ...
  **Parallelization**:
  - **Can Run In Parallel**: YES
  - **Parallel Group**: Wave 2 (depends on Task 5)
  - **Blocks**: Task 11
  - **Blocked By**: Task 5
  ...
  **Pattern References**:
  - `src/Bookkeeping.App/Views/ImportView.axaml` — 现有弹窗和对话框样式参考
  - `src/Bookkeeping.App/App.axaml` — 主题资源

  **API/Type References**:
  - `src/Bookkeeping.App/ViewModels/CategoriesViewModel.cs` — 主 ViewModel，弹窗需要与之交互
  ...
  **QA Scenarios (MANDATORY)**:
  ...
  Scenario: 编辑分类名称
    Tool: interactive_bash (dotnet run)
    Preconditions: 分类 "餐饮美食" 存在
    Steps:
      1. 点击 "餐饮美食" 的编辑按钮
      2. 验证弹窗显示当前名称 "餐饮美食" 和图标
      3. 修改名称为 "餐饮美食2"
      4. 点击确认
      5. 验证列表中名称已更新
      6. 重启应用验证持久化
    Expected Result: 名称修改成功并持久化
    Failure Indicators: 名称未更改、修改后重启丢失
  ...
  Scenario: 编辑分类 - 空名称验证
    ...
    Steps:
      ...
      3. 点击确认
      4. 验证显示验证错误 "名称不能为空"
    Expected Result: 确认按钮禁用或显示验证错误
    Failure Indicators: 空名称成功保存
    Evidence: .sisyphus/evidence/task-7-empty-name.png
  ```

  **Commit**: YES (group with Wave 2)
  - Message: `feat(category): add category edit dialog with validation`
  - Files: `src/Bookkeeping.App/Views/CategoryEditDialog.axaml`, `src/Bookkeeping.App/Views/CategoryEditDialog.axaml.cs`, `src/Bookkeeping.App/ViewModels/CategoryEditViewModel.cs`

- [ ] 8. EmojiPickerDialog 弹窗

  **What to do**:
  - 创建 `src/Bookkeeping.App/Views/EmojiPickerDialog.axaml` 和 `.axaml.cs`
  - 使用 Task 3 创建的 EmojiData 数据集
  - 弹窗布局：
    - 顶部：分类 Tab（餐饮、交通、金融、常用等）
    - 中部：Emoji 网格（每行 6-8 个 Emoji，点击可选中）
    - 底部：确认/取消按钮
    - 选中状态高亮
  - 集成到 CategoryEditDialog：
    - 在分类编辑弹窗中，图标区域显示当前 Emoji
    - 点击图标区域触发 EmojiPickerDialog
    - 选择后更新 SelectedIcon
  - 弹窗以 Avalonia Window 形式弹出，返回选中的 Emoji 字符串
  ...
  **Parallelization**:
  ...
  - **Blocks**: Task 7 (CategoryEditDialog 需要集成)
  - **Blocked By**: Task 3 (EmojiData), Task 5 (ViewModel)

  **References**:

  **Pattern References**:
  - `src/Bookkeeping.App/Models/EmojiData.cs` — Emoji 数据集（Task 3 产出）

  **API/Type References**:
  - `src/Bookkeeping.Core/Models/Category.cs` — Icon 字段（存 emoji 字符串）
  ...
  **QA Scenarios (MANDATORY)**:
  ...
  ```
  Scenario: Emoji 选择器弹窗 - 选择和确认
    Tool: interactive_bash (dotnet run)
    Steps:
      1. 点击分类编辑中的图标选择器
      2. 验证 Emoji 弹窗显示，包含分类 Tab
      3. 切换到 "餐饮" 分类
      4. 点击 🍔 Emoji
      5. 验证弹窗关闭，图标显示为 🍔
    Expected Result: Emoji 选择成功，正确显示
    Failure Indicators: 弹窗不显示、Emoji 不响应点击、选择后图标不更新
    Evidence: .sisyphus/evidence/task-8-emoji-picker.png
  ```

  **Commit**: YES (group with Wave 2)
  - Message: `feat(category): add emoji picker dialog`
  - Files: `src/Bookkeeping.App/Views/EmojiPickerDialog.axaml`, `src/Bookkeeping.App/Views/EmojiPickerDialog.axaml.cs`

- [ ] 9. 删除重新分配对话框

  **What to do**:
  - 创建 `src/Bookkeeping.App/Views/DeleteReassignDialog.axaml` 和 `.axaml.cs`
  - 创建 `src/Bookkeeping.App/ViewModels/DeleteReassignViewModel.cs`
  - 弹窗内容：
    - 提示信息："分类 '{name}' 下有 {count} 条交易记录，请选择目标分类进行转移"
    - 目标分类下拉框（同类型分类列表，排除被删除分类）
    - 不允许选择被删除分类自身（禁用确认按钮）
    - 确认/取消按钮
  - 逻辑：
    - 加载同类型分类列表（排除当前删除的分类）
    - 确认时调用 CategoryService.ReassignAndDeleteAsync(deleteId, targetId)
    - 取消时关闭弹窗，不执行任何操作
    - 如果某类型只剩一个分类，删除按钮应禁用（ViewModel 层已在主页面处理）
  - 关闭弹窗时返回结果（bool 表示是否确认，long 表示 targetCategoryId）

  **Must NOT do**:
  - 不允许选择不同类型的分类作为目标（只能同类型）
  - 不允许选择被删除的分类自身
  - 不直接删除交易记录
  ...
  **Pattern References**:
  - `src/Bookkeeping.App/Views/ImportView.axaml` — 参考弹窗样式和交互模式

  **API/Type References**:
  - `src/Bookkeeping.Core/Services/ICategoryService.cs` — ReassignAndDeleteAsync 方法签名
  - `src/Bookkeeping.Core/Models/Category.cs` — Category 类型（用于筛选同类型分类）
  ...
  **QA Scenarios (MANDATORY)**:
  ...
  ```
  Scenario: 删除分类 - 重新分配对话框
    Tool: interactive_bash (dotnet run)
    Preconditions: "餐饮美食" 下有 5 条交易，同类型还有 "购物消费" 分类
    Steps:
      1. 点击 "餐饮美食" 的删除按钮
      2. 验证弹出重新分配对话框
      3. 验证下拉框只包含同类型（支出）分类
      ...
      6. 点击确认
      7. 验证 "餐饮美食" 从列表中消失
      8. 查看 "购物消费" 下现在有 5 条交易
    Expected Result: 删除成功，交易正确转移
    Failure Indicators: 下拉框包含不同类型分类、被删分类出现在下拉框中、转移后交易数不对
    Evidence: .sisyphus/evidence/task-9-delete-reassign.png

  Scenario: 删除最后一个分类 - 禁止删除
    Tool: interactive_bash (dotnet run)
    Preconditions: 支出类型只剩 1 个分类
    Steps:
      1. 点击该分类的删除按钮
      2. 验证显示提示 "至少保留一个分类" 或删除按钮被禁用
    Expected Result: 删除被阻止
    Failure Indicators: 删除成功导致某类型无分类
    Evidence: .sisyphus/evidence/task-9-delete-last.png
  ```

  **Commit**: YES (group with Wave 2)
  - Message: `feat(category): add delete reassign dialog`
  - Files: `src/Bookkeeping.App/Views/DeleteReassignDialog.axaml`, `src/Bookkeeping.App/Views/DeleteReassignDialog.axaml.cs`, `src/Bookkeeping.App/ViewModels/DeleteReassignViewModel.cs`

- [ ] 10. 分类统计页面（ViewModel + View）
